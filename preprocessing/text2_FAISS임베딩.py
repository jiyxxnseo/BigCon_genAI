...
# CSV 파일 로드 및 인덱스 생성
file_path = "/content/restaurant_info_data_with_text2.csv"  # Google Colab에 업로드한 경로
restaurant_data = pd.read_csv(file_path, usecols=['restaurant_name', 'text2'])
print(restaurant_data.head())

# FAISS 인덱스 빌드
faiss_index, embeddings = build_faiss_index(restaurant_data['text2'].fillna("정보 없음").tolist())

# FAISS 인덱스 저장
faiss.write_index(faiss_index, '/content/restaurant_faiss.index')
np.save('/content/restaurant_embeddings.npy', embeddings)
